...
class fileNode():
    ...
    def __init__(self, name, color, type, modTime):
        self.value_ = name
...
class headInGraph():
    ...
    def addNode(self, name, color,type, modTime):
        self.nodePositions_[name] = len(self.nodes_)
        self.nodes_.append(fileNode(name,color,type, modTime))
    ...
    def printGraphViz(self, out, title, outPutExternal):
        ...
        for nPos in range(len(self.nodes_)):
            if(not outPutExternal and self.nodes_[nPos].type_ == "external"):
                continue
            out.write(self.nodes_[nPos].value_ + "[shape=circle,style=filled,fixedsize =false, color = \"#000000\", fillcolor =\"" + self.nodes_[nPos].color_ + "\", width = 1]\n") 
        for nPos in range(len(self.nodes_)):
            if(not outPutExternal and self.nodes_[nPos].type_ == "external"):
                continue
