...
void profilerSetUp::processHeader() { setOption(header_, "-header", "header"); }
...
void profilerSetUp::processExtra() {
  if (setOption(extraInfoStr_, "-extra,-extraCol,-extraInfo", "extra")) {
    ...
    for (const auto& tok : toks) {
      ...
      if (subToks.size() != 2) {
        std::cout << "Error, extra info needs to two strings separated by a "
                     "colon, eg. COLNAME:COLINFO" << std::endl;
